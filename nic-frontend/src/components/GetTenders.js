...
const GetTenders = () => {
 ...
 const downloadAllTenders = async () => {
  try {
    const response = await API.get("download-all-tenders/", {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", "all_tenders.xlsx");
    document.body.appendChild(link);
    link.click();
    link.remove();
  } catch (error) {
    console.error(error);
    alert("❌ Failed to download all tenders");
  }
};
